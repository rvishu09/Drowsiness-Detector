...
export default function Detection() {
  ...
  const handleResults = useCallback((results) => {
    ...
    const headTilt = Math.abs(
      Math.atan2(chin.x - nose.x, chin.y - nose.y) *
        (180 / Math.PI)
    );
    ...
    const cooldownOk =
      now - lastAlert.current > COOLDOWN_MS;

    if (
      drowsyFrames.current >= CONSECUTIVE_FRAMES &&
      cooldownOk
    ) {
  ...
  useEffect(() => {
    ...
    async function initMediaPipe() {
      try {
        ...
        setCamError(
          'Failed to load face detection model. Make sure you have internet connection and refresh the page.'
        );
            ...
            {metrics && (
              <div style={styles.metricsBox}>
                <MetricRow
                  label="EAR"
                  value={metrics.ear}
                  warn={metrics.isDrowsy}
                />

                <MetricRow
                  label="MAR"
                  value={metrics.mar}
                  warn={metrics.isYawning}
                />

                <MetricRow
                  label="Head Tilt"
                  value={metrics.headTilt + '°'}
                  warn={metrics.headTilt > 20}
                />
              </div>
            )}
